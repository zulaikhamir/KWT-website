...
export default function SpeakerCard({ person }: SpeakerCardProps) {
  ...
  const description = person.description ?? "Sharing knowledge and experiences with the KWT community.";
  ...
  return (
    ...
    >
      {!person.photo || imgFailed ? (
        <div aria-hidden="true" className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-linear-to-b from-[var(--color-primary)] to-[#0E1730] transition-opacity duration-500 group-hover:opacity-40 group-focus-within:opacity-40 motion-reduce:transition-none">
          ...
          <span className="select-none font-heading text-2xl font-bold tracking-tight text-white/20">{initialFor(person.name)}</span>
        </div>
      ) : (
          <img
            src={person.photo}
      ...
      {person.linkedinUrl && (
        <a href={person.linkedinUrl} target="_blank" rel="noreferrer noopener" aria-label={`${person.name} on LinkedIn`} className="absolute right-2.5 top-2.5 z-20 flex size-8 items-center justify-center rounded-full border border-white/30 bg-black/30 text-white backdrop-blur-md transition-colors hover:border-white/60 hover:bg-black/55 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/70">
